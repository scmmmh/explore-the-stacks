...
def shelf_group(request, shelf, prev, nxt):
    ...
    if 'q' in request.params and request.params['q'].strip():
        ...
        body = {'query': {'match': {'_all': request.params['q'].strip()}},
                ...
                'size': shelves.count()}
        matches = list(set([int(d['_id']) for d in es.search(index='ets',
                                                        doc_type='shelf',
                                                        body=body)['hits']['hits']]))
    else:
        matches = []
...
@view_config(route_name='shelf', renderer='ets:templates/view.kajiki')
def shelves(request):
    dbsession = DBSession()
    shelf = dbsession.query(Shelf).filter(Shelf.id==request.matchdict['sid']).first()
    if shelf and shelf.order is not None:
